...
#Adapted from code provided by Hernan et al. 2020
...
#########################################
# Calculating Risk Differences by Sex
...
#########################################
# Male Only for Simulated Exposure
# Female exposed to true NDVI value
#########################################
...
#########################################
# Female Only for Simulated Exposure
# Male exposed to true NDVI value
#########################################
...
standardization <- function(data, indices) {
  ...
  d1 <- d # 2nd copy (Minimum): intervention set to 1, outcome to missing
  d1$interv <- 1
  d1$intervention_b [d1$sex == "Female"] <- -.0815979465842247
  d1$glucose_mgdl <- NA
  
  d2 <- d # 3rd copy (10th percentile): intervention set to 2, outcome to missing
  d2$interv <- 2
  d2$intervention_b  [d2$sex == "Female"]<- .086611807346344
  d2$glucose_mgdl <- NA
  
  d3 <- d # 4th copy (20th percentile): intervention set to 3, outcome to missing
  d3$interv <- 3
  d3$intervention_b [d3$sex == "Female"] <- .1143549978733063
  d3$glucose_mgdl <- NA
  
  d4 <- d # 5th copy (30th percentile): intervention set to 4, outcome to missing
  d4$interv <- 4
  d4$intervention_b [d4$sex == "Female"] <- .1339770704507828
  d4$glucose_mgdl <- NA
  
  d5 <- d # 6th copy (40th percentile): intervention set to 5, outcome to missing
  d5$interv <- 5
  d5$intervention_b [d5$sex == "Female"] <- .1468733251094818
  d5$glucose_mgdl <- NA
  
  d6 <- d # 7th copy (50th percentile): intervention set to 6, outcome to missing
  d6$interv <- 6
  d6$intervention_b  [d6$sex == "Female"]<- .1663886681199074
  d6$glucose_mgdl <- NA
  
  d7 <- d # 8th copy (60th percentile): intervention set to 7, outcome to missing
  d7$interv <- 7
  d7$intervention_b  [d7$sex == "Female"]<- .1879086345434189
  d7$glucose_mgdl <- NA
  
  d8 <- d # 9th copy (70th percentile): intervention set to 8, outcome to missing
  d8$interv <- 8
  d8$intervention_b  [d8$sex == "Female"]<- .2076895833015442
  d8$glucose_mgdl <- NA
  
  d9 <- d # 10th copy (80th percentile): intervention set to 9, outcome to missing
  d9$interv <- 9
  d9$intervention_b [d9$sex == "Female"] <- .2258577197790146
  d9$glucose_mgdl <- NA
  
  d10 <- d # 11th copy (90th percentile): intervention set to 10, outcome to missing
  d10$interv <- 10
  d10$intervention_b [d10$sex == "Female"] <- .2258577197790146
  d10$glucose_mgdl <- NA
  
  d11 <- d # 12th copy (Maximum): intervention set to 11, outcome to missing
  d11$interv <- 11
  d11$intervention_b  [d11$sex == "Female"]<- .3624528
  d11$glucose_mgdl <- NA
